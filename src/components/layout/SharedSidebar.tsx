...
export function SharedSidebar({
    ...
    return (
      <Link
        ...
        style={{
          ...
          borderLeft: showLabel ? `2px solid ${active ? accentColor : "transparent"}` : "none",
      ...
      >
        ...
        {item.badge ? (
          <span
            style={{
              marginLeft: showLabel ? "auto" : undefined,
              minWidth: 18,
              height: 18,
              borderRadius: 9,
  ...
  const FooterActions = ({ showLabels = true }: { showLabels?: boolean }) => (
    ...
    >
      ...
      >
        ...
        {showLabels && (isDark ? "Dark Mode" : "Light Mode")}
      </button>
      <button
        onClick={logout}
        style={{
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10, overflow: "hidden", flex: 1 }}>
          {/* Hamburger — opens full overlay drawer on ALL sizes */}
          <button
  ...
  // ── Overlay Drawer — slides in from left on ALL screen sizes ─────────────
  const OverlayDrawer = () => (
    <>
      {/* Backdrop */}
      {drawerOpen && (
