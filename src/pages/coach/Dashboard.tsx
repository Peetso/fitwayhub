...
export default function CoachHome() {
  ...
  const getActivityIcon = (item: ActivityItem) => {
    if (item.type === "booking") return { icon: ClipboardList, color: "var(--amber)", text: item.status === "pending" ? `New coaching request from ${item.actor_name}` : `Booking ${item.status} for ${item.actor_name}` };
    if (item.type === "message") return { icon: MessageSquare, color: "var(--blue)", text: `${item.actor_name} sent you a message` };
    if (item.type === "review") return { icon: Star, color: "var(--amber)", text: `New ${item.rating}★ review from ${item.actor_name}` };
    return { icon: CheckCircle, color: "var(--accent)", text: "Activity" };
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: "var(--text-secondary)", marginTop: 3 }}>Here's your coaching overview for this week</p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 12 }}>
        {[
          { label: "My Athletes", value: stats?.athletes ?? 0, icon: Users, color: "var(--blue)", link: "/coach/athletes" },
          { label: "Pending Requests", value: stats?.pendingRequests ?? 0, icon: ClipboardList, color: "var(--amber)", link: "/coach/requests" },
          { label: "Sessions / Week", value: stats?.sessionsThisWeek ?? 0, icon: Activity, color: "var(--accent)", link: "/coach/athletes" },
          { label: "Monthly Revenue", value: `${(stats?.monthlyRevenue ?? 0).toFixed(0)} EGP`, icon: DollarSign, color: "var(--cyan)", link: "/coach/ads" },
          { label: "Avg Rating", value: `${stats?.avgRating ?? "—"}★`, icon: Star, color: "var(--amber)", link: "/coach/profile" },
          { label: "Completion %", value: `${stats?.completionRate ?? 0}%`, icon: TrendingUp, color: "var(--accent)", link: "/coach/athletes" },
        ].map((s) => (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 16 }}>
        ...
        <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 16, padding: "20px 22px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
            <p style={{ fontFamily: "'Chakra Petch', sans-serif", fontSize: 14, fontWeight: 700 }}>Sessions This Week</p>
            <span style={{ fontSize: 12, color: "var(--blue)", fontWeight: 600 }}>{stats?.sessionsThisWeek ?? 0} total</span>
          </div>
        ...
        <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 16, padding: "20px 22px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
            <p style={{ fontFamily: "'Chakra Petch', sans-serif", fontSize: 14, fontWeight: 700 }}>Upcoming Sessions</p>
            <Link to="/coach/requests" style={{ fontSize: 12, color: "var(--accent)", textDecoration: "none", fontWeight: 600 }}>View all</Link>
          </div>
          {sessions.length === 0 ? (
            <p style={{ fontSize: 13, color: "var(--text-muted)", textAlign: "center", paddingTop: 20 }}>No upcoming sessions</p>
            ...
            <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: i < Math.min(sessions.length, 3) - 1 ? "1px solid var(--border)" : "none" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2 }}>{s.date || "TBD"} {s.time || ""} · {s.booking_type || "Session"}</p>
              </div>
      ...
      <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 16, padding: "20px 22px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
          <p style={{ fontFamily: "'Chakra Petch', sans-serif", fontSize: 14, fontWeight: 700 }}>Recent Activity</p>
          <span style={{ fontSize: 12, color: "var(--text-muted)" }}>Live</span>
        </div>
        {activity.length === 0 ? (
          <p style={{ fontSize: 13, color: "var(--text-muted)", textAlign: "center", paddingTop: 12 }}>No recent activity yet</p>
        ) : activity.map((a, i) => {
