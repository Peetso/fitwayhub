...
interface ChatUser { id: number; name: string; email: string; avatar: string; role: string; }
...
export default function CoachChat() {
  ...
  useEffect(() => {
    api("/api/chat/users")
      .then(r => r.json())
      .then(d => setChatUsers((d.users || []).filter((u: ChatUser) => u.role !== "admin")))
      .catch(() => {})
    ...
    <div style={{ display: "flex", gap: 0, height: "calc(100dvh - 120px)", backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: isMobile ? 0 : 16, overflow: "hidden" }}>
      <div style={{ width: showList ? (isMobile ? "100%" : 280) : 0, borderRight: isMobile ? "none" : "1px solid var(--border)", display: showList ? "flex" : "none", flexDirection: "column", flexShrink: 0 }}>
        <div style={{ padding: "16px", borderBottom: "1px solid var(--border)" }}>
          <p style={{ fontFamily: "'Chakra Petch', sans-serif", fontSize: 14, fontWeight: 700, marginBottom: 10 }}>Messages</p>
        ...
        <div style={{ overflowY: "auto", flex: 1 }}>
          {loading ? (
            ...
            <p style={{ padding: 20, textAlign: "center", color: "var(--text-muted)", fontSize: 13 }}>No users yet</p>
          ) : filtered.map(u => (
            <button key={u.id} onClick={() => setActiveUser(u)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, padding: "13px 16px", borderBottom: "1px solid var(--border)", background: activeUser?.id === u.id ? "var(--accent-dim)" : "none", border: "none", cursor: "pointer", textAlign: "left", borderLeft: activeUser?.id === u.id ? "3px solid var(--blue)" : "3px solid transparent", transition: "all 0.15s" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2, textTransform: "capitalize" }}>{u.role}</p>
              </div>
            </button>
          ))}
        </div>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
          <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 11, color: "var(--accent)" }}>● Online</p>
            </div>
          </div>
          <div style={{ flex: 1, overflowY: "auto", padding: "20px", display: "flex", flexDirection: "column", gap: 12 }}>
            {messages.length === 0 && <p style={{ textAlign: "center", color: "var(--text-muted)", fontSize: 13, paddingTop: 40 }}>No messages yet. Start the conversation!</p>}
            {messages.map(msg => {
          ...
          <div style={{ padding: "14px 16px", borderTop: "1px solid var(--border)", display: "flex", gap: 10, alignItems: "flex-end" }}>
            <input className="input-base" value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && !e.shiftKey && sendMsg()} placeholder="Type a message..." style={{ flex: 1, padding: "10px 14px", fontSize: 13 }} />
      ...
      {!isMobile && !activeUser && (
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--text-muted)", flexDirection: "column", gap: 10 }}>
          <p style={{ fontFamily: "'Chakra Petch', sans-serif", fontSize: 15 }}>Select a conversation</p>
          <p style={{ fontSize: 13 }}>Choose a user from the list to start chatting</p>
        </div>
      )}
    </div>
  );
