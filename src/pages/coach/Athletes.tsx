...
export default function CoachAthletes() {
  ...
  const saveWorkout = async () => {
    ...
    try {
      ...
      if (r.ok) showMsg("✅ Workout plan saved!"); else showMsg("❌ Failed to save.");
  ...
  const saveNutrition = async () => {
    ...
    try {
      ...
      if (r.ok) showMsg("✅ Nutrition plan saved!"); else showMsg("❌ Failed to save.");
    } catch { showMsg("❌ Failed to save."); }
    finally { setSaving(false); }
  };
      ...
      <div style={{ width: isMobile ? "100%" : 260, flexShrink: 0, display: showAthleteList ? "block" : "none" }}>
        <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 16, overflow: "hidden", position: "sticky", top: 20 }}>
          <div style={{ padding: "14px 16px", borderBottom: "1px solid var(--border)" }}>
            <p style={{ fontFamily: "'Chakra Petch', sans-serif", fontSize: 14, fontWeight: 700, marginBottom: 10 }}>My Athletes</p>
            <div style={{ position: "relative" }}>
              <Search size={13} style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", color: "var(--text-muted)" }} />
              <input className="input-base" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search..." style={{ paddingLeft: 30, padding: "7px 10px 7px 30px", fontSize: 12 }} />
            </div>
          </div>
          <div style={{ maxHeight: "calc(100dvh - 260px)", overflowY: "auto" }}>
            {loading ? (
              ...
              <p style={{ padding: 20, textAlign: "center", color: "var(--text-muted)", fontSize: 13 }}>No athletes registered</p>
            ) : filtered.map(a => (
              <button key={a.id} onClick={() => selectAthlete(a)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "12px 14px", borderBottom: "1px solid var(--border)", background: selected?.id === a.id ? "var(--accent-dim)" : "none", border: "none", cursor: "pointer", textAlign: "left", borderLeft: selected?.id === a.id ? "3px solid var(--blue)" : "3px solid transparent", transition: "all 0.15s" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 1 }}>{a.steps?.toLocaleString() || 0} steps</p>
                </div>
      ...
      <div style={{ flex: 1, minWidth: 0, display: showAthleteDetail ? "block" : "none" }}>
        {!selected ? (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: 300, flexDirection: "column", gap: 12, color: "var(--text-muted)" }}>
            ...
            <p style={{ fontFamily: "'Chakra Petch', sans-serif", fontSize: 15 }}>Select an athlete to manage their plans</p>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 16, padding: "18px 22px" }}>
              {isMobile && (
                <button onClick={() => setSelected(null)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-secondary)", padding: "0 0 12px 0", fontSize: 13, display: "flex", alignItems: "center", gap: 6 }}>← Back to Athletes</button>
              )}
              <div style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {(["overview","workout","nutrition"] as const).map(t => (
                    <button key={t} onClick={() => setTab(t)} style={{ padding: "7px 14px", borderRadius: 8, border: `1px solid ${tab === t ? "var(--blue)" : "var(--border)"}`, background: tab === t ? "rgba(59,139,255,0.12)" : "var(--bg-surface)", color: tab === t ? "var(--blue)" : "var(--text-secondary)", fontSize: 12, fontWeight: tab === t ? 600 : 400, cursor: "pointer", textTransform: "capitalize", fontFamily: "'Chakra Petch', sans-serif" }}>{t}</button>
                  ))}
                </div>
              </div>
            </div>
            ...
            {tab === "overview" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 12 }}>
                  {[
                    { label: "Height", value: selected.height ? `${selected.height} cm` : "—", color: "var(--blue)" },
                    { label: "Weight", value: selected.weight ? `${selected.weight} kg` : "—", color: "var(--amber)" },
                    { label: "BMI", value: bmi || "—", color: "var(--cyan)" },
                    { label: "Steps Today", value: (selected.steps || 0).toLocaleString(), color: "var(--accent)" },
                    { label: "Premium", value: selected.is_premium ? "Yes" : "Free", color: selected.is_premium ? "var(--accent)" : "var(--text-muted)" },
                  ].map(s => (
                ...
                <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 14, padding: "16px 18px" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      <p style={{ fontFamily: "'Chakra Petch', sans-serif", fontSize: 13, fontWeight: 700 }}>Daily Step Goal</p>
                    </div>
                    {!editingStepGoal && (
                  ...
                  {editingStepGoal ? (
                    ...
                    <p style={{ fontFamily: "'Chakra Petch', sans-serif", fontSize: 22, fontWeight: 700, color: "var(--accent)" }}>
                      {(selected.step_goal || 10000).toLocaleString()} <span style={{ fontSize: 13, fontWeight: 400, color: "var(--text-muted)" }}>steps/day</span>
                    </p>
                  )}
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  {[{ label: "Workout Plan", tab: "workout" as const, color: "var(--accent)", bg: "var(--accent-dim)", border: "rgba(200,255,0,0.2)" }, { label: "Nutrition Plan", tab: "nutrition" as const, color: "var(--blue)", bg: "rgba(59,139,255,0.1)", border: "rgba(59,139,255,0.25)" }].map(p => (
                    <div key={p.label} style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 14, padding: "16px 18px" }}>
                      ...
                      <p style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 10 }}>{p.tab === "workout" ? `${workoutPlan.exercises.length} exercises` : `${nutritionPlan.daily_calories} kcal/day`}</p>
                      <button onClick={() => setTab(p.tab)} style={{ width: "100%", padding: "7px", borderRadius: 8, background: p.bg, border: `1px solid ${p.border}`, color: p.color, cursor: "pointer", fontSize: 12, fontWeight: 600 }}>Manage Plan</button>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {tab === "workout" && (
              <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 16, padding: "18px 22px", display: "flex", flexDirection: "column", gap: 14 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <p style={{ fontFamily: "'Chakra Petch', sans-serif", fontSize: 15, fontWeight: 700 }}>Workout Plan</p>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 12 }}>
                  <div>
                    <label style={{ fontSize: 11, color: "var(--text-muted)", display: "block", marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.07em" }}>Plan Title</label>
                    <input className="input-base" value={workoutPlan.title} onChange={e => setWorkoutPlan(p => ({ ...p, title: e.target.value }))} />
                  </div>
                  <div>
                    <label style={{ fontSize: 11, color: "var(--text-muted)", display: "block", marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.07em" }}>Days/Week</label>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <p style={{ fontSize: 13, fontWeight: 600 }}>Exercises ({workoutPlan.exercises.length})</p>
                  <button onClick={addExercise} style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 12px", borderRadius: 7, background: "var(--accent-dim)", border: "1px solid rgba(200,255,0,0.25)", color: "var(--accent)", fontSize: 12, cursor: "pointer", fontWeight: 600 }}><Plus size={13} /> Add Exercise</button>
                </div>
                {workoutPlan.exercises.length === 0 && <p style={{ textAlign: "center", padding: "24px 0", color: "var(--text-muted)", fontSize: 13 }}>No exercises yet. Click "Add Exercise" to begin.</p>}
            ...
            {tab === "nutrition" && (
              <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 16, padding: "18px 22px", display: "flex", flexDirection: "column", gap: 14 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <p style={{ fontFamily: "'Chakra Petch', sans-serif", fontSize: 15, fontWeight: 700 }}>Nutrition Plan</p>
                ...
                <input className="input-base" value={nutritionPlan.title} onChange={e => setNutritionPlan(p => ({ ...p, title: e.target.value }))} placeholder="Plan title" />
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <p style={{ fontSize: 13, fontWeight: 600 }}>Meals ({nutritionPlan.meals.length})</p>
                  <button onClick={addMeal} style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 12px", borderRadius: 7, background: "rgba(59,139,255,0.1)", border: "1px solid rgba(59,139,255,0.25)", color: "var(--blue)", fontSize: 12, cursor: "pointer", fontWeight: 600 }}><Plus size={13} /> Add Meal</button>
                </div>
                {nutritionPlan.meals.length === 0 && <p style={{ textAlign: "center", padding: "24px 0", color: "var(--text-muted)", fontSize: 13 }}>No meals yet.</p>}
                ...
                <div>
                  <label style={{ fontSize: 11, color: "var(--text-muted)", display: "block", marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.07em" }}>Coach Notes</label>
                  <textarea className="input-base" value={nutritionPlan.notes} onChange={e => setNutritionPlan(p => ({ ...p, notes: e.target.value }))} placeholder="Special instructions..." rows={3} style={{ resize: "none" }} />
                </div>
              </div>
            )}
          </div>
