    ...
    <div style={{ display: "flex", gap: 20, alignItems: "flex-start" }}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 16, minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <h1 style={{ fontFamily: "'Chakra Petch', sans-serif", fontSize: "clamp(18px,4vw,26px)", fontWeight: 700 }}>Community</h1>
            <p style={{ fontSize: 13, color: "var(--text-secondary)", marginTop: 3 }}>Share insights and connect with the community</p>
          </div>
        ...
        <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 14, padding: "14px 16px", display: "flex", gap: 12, alignItems: "center" }}>
          ...
          <button onClick={() => setShowCompose(true)} style={{ flex: 1, textAlign: "left", padding: "10px 14px", borderRadius: 10, background: "var(--bg-surface)", border: "1px solid var(--border)", color: "var(--text-muted)", cursor: "pointer", fontSize: 13 }}>
            Share an insight, tip, or success story...
          </button>
        </div>

        {loading ? (
          <div style={{ padding: 40, textAlign: "center", color: "var(--text-muted)", fontSize: 14 }}>Loading posts...</div>
        ) : posts.length === 0 ? (
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                    ...
                    <span style={{ fontSize: 11, color: post.user_role === "coach" ? "var(--blue)" : "var(--text-muted)", backgroundColor: post.user_role === "coach" ? "rgba(59,139,255,0.1)" : "var(--bg-surface)", padding: "2px 8px", borderRadius: 20, border: `1px solid ${post.user_role === "coach" ? "rgba(59,139,255,0.2)" : "var(--border)"}` }}>
                      {post.user_role === "coach" ? "Coach" : "Athlete"}
                    </span>
                  </div>
      ...
      <div className="hidden lg:flex" style={{ width: 260, flexDirection: "column", gap: 14, flexShrink: 0, position: "sticky", top: 20 }}>
        ...
        <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 16, padding: "18px 18px" }}>
          <p style={{ fontFamily: "'Chakra Petch', sans-serif", fontSize: 13, fontWeight: 700, marginBottom: 14 }}>Community Stats</p>
          {[{ label: "Total Posts", value: posts.length, icon: MessageSquare }, { label: "Coach Posts", value: coachPosts.length, icon: Users }, { label: "Total Likes", value: posts.reduce((s, p) => s + p.likes, 0), icon: TrendingUp }].map(s => (
        ...
        {topTags.length > 0 && (
          <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 16, padding: "18px" }}>
            <p style={{ fontFamily: "'Chakra Petch', sans-serif", fontSize: 13, fontWeight: 700, marginBottom: 12 }}>Trending Topics</p>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 1000, backgroundColor: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 20, padding: 28, width: "100%", maxWidth: 520, maxHeight: "90dvh", overflowY: "auto" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
              <p style={{ fontFamily: "'Chakra Petch', sans-serif", fontSize: 16, fontWeight: 700 }}>Create Post</p>
            ...
            <div style={{ display: "flex", gap: 12, marginBottom: 14 }}>
              ...
              <div style={{ flex: 1 }}>
                <p style={{ fontFamily: "'Chakra Petch', sans-serif", fontSize: 13, fontWeight: 700 }}>{user?.name || "Coach"}</p>
                <p style={{ fontSize: 11, color: "var(--blue)" }}>Fitness Coach</p>
              </div>
            </div>
            <textarea className="input-base" value={newContent} onChange={e => setNewContent(e.target.value)} placeholder="Share an insight, celebrate a win, or ask the community..." rows={5} style={{ resize: "none", marginBottom: 12, fontSize: 14 }} autoFocus />
            <div>
              <label style={{ fontSize: 11, color: "var(--text-muted)", display: "block", marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.07em" }}>Tags (e.g. #strength #nutrition)</label>
              <input className="input-base" value={newTags} onChange={e => setNewTags(e.target.value)} placeholder="#strength #nutrition" style={{ marginBottom: 16 }} />
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setShowCompose(false)} style={{ flex: 1, padding: "11px", borderRadius: 10, background: "var(--bg-surface)", border: "1px solid var(--border)", color: "var(--text-secondary)", cursor: "pointer", fontSize: 13 }}>Cancel</button>
