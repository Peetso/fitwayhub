...
import { Users, Dumbbell, DollarSign, Activity, TrendingUp, Trash2, Shield, UserCheck, Gift, Sun, Moon, Plus, X, Search, Video, Megaphone, Star, CheckCircle, Clock, CreditCard, Play, Lock, Unlock, Save, Eye, EyeOff } from "lucide-react";
import { useAuth } from "@/context/AuthContext";
import { useTheme } from "@/context/ThemeContext";
import { useI18n } from "@/context/I18nContext";
import { useLocation } from "react-router-dom";

interface AdminUser { id: number; name: string; email: string; role: string; is_premium: boolean; membership_paid?: boolean; points: number; steps: number; created_at: string; avatar: string; }
...
type Tab = "overview" | "users" | "coaches" | "payments" | "videos" | "ads" | "gifts" | "community" | "app-config" | "website" | "settings" | "subscriptions" | "withdrawals";
    ...
    try {
      ...
      if (usersData.users) {
        ...
        setStats({
          ...
          premium: usersData.users.filter((u: AdminUser) => u.is_premium).length,
          revenue: 0,
        });
      }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: "var(--text-secondary)", marginTop: 2 }}>Manage your platform</p>
        </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16 }}>
            <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 16, padding: "20px 22px" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                <p style={{ fontFamily: "'Chakra Petch', sans-serif", fontSize: 14, fontWeight: 700 }}>Recent Users</p>
                <TrendingUp size={16} color="var(--red)" />
              </div>
              {users.length === 0 ? <p style={{ fontSize: 13, color: "var(--text-muted)" }}>No users yet</p> : users.slice(0, 5).map((u, i) => (
                <div key={u.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 0", borderBottom: i < 4 ? "1px solid var(--border)" : "none" }}>
                  ...
                  <span style={{ fontSize: 11, color: u.is_premium ? "var(--accent)" : "var(--text-muted)" }}>{u.is_premium ? "⚡ Premium" : "Free"}</span>
                </div>
              ))}
            </div>
            <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 16, padding: "20px 22px" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                <p style={{ fontFamily: "'Chakra Petch', sans-serif", fontSize: 14, fontWeight: 700 }}>Recent Payments</p>
                <CreditCard size={16} color="var(--accent)" />
              </div>
              {payments.length === 0 ? <p style={{ fontSize: 13, color: "var(--text-muted)" }}>No payments yet</p> : payments.slice(0, 5).map((p, i) => (
              ...
              <tbody>
                {filtered.map((u, i) => (
                  <tr key={u.id} style={{ borderBottom: i < filtered.length - 1 ? "1px solid var(--border)" : "none" }}>
                    ...
                    <td style={{ padding: "12px 16px" }}>
                      {u.role === "coach" ? (
                        <button onClick={async () => { const paid = !(u as any).membership_paid; await api(`/api/admin/users/${u.id}/coach-membership`, { method: "PATCH", body: JSON.stringify({ membership_paid: paid }) }); fetchAll(); showMsg(paid ? "✅ Coach membership activated" : "Coach membership revoked"); }} style={{ padding: "4px 10px", borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: "pointer", border: `1px solid ${(u as any).membership_paid ? "var(--cyan)" : "rgba(255,68,68,0.4)"}`, background: (u as any).membership_paid ? "rgba(6,182,212,0.1)" : "rgba(255,68,68,0.08)", color: (u as any).membership_paid ? "var(--cyan)" : "var(--red)" }}>
                          {(u as any).membership_paid ? "✓ Active" : "✗ Inactive"}
                        </button>
                    ...
                    <td style={{ padding: "12px 16px" }}>
                      <div style={{ display: "flex", gap: 6 }}>
                        <button onClick={() => { setGiftForm(f => ({ ...f, user_id: u.id })); setShowGiftModal(true); setTab("gifts"); }} style={{ padding: "5px 8px", borderRadius: 6, background: "rgba(255,179,64,0.1)", border: "1px solid var(--amber)", color: "var(--amber)", cursor: "pointer" }}>
                          <Gift size={13} />
                        </button>
                        <button onClick={() => deleteUser(u.id)} style={{ padding: "5px 8px", borderRadius: 6, background: "rgba(255,68,68,0.1)", border: "1px solid var(--red)", color: "var(--red)", cursor: "pointer" }}>
                          <Trash2 size={13} />
                        </button>
                      </div>
                    </td>
      ...
      {tab === "coaches" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <p style={{ fontFamily: "'Chakra Petch', sans-serif", fontSize: 16, fontWeight: 700 }}>Manage Coaches</p>
                ...
                <div key={c.id} style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 16, padding: 20 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    <button onClick={async () => {
                      ...
                      fetchAll();
                      showMsg(paid ? "✅ Coach membership activated" : "Coach membership deactivated");
                      ...
                      {(c as any).membership_paid ? "✓ Active" : "✗ Inactive"}
                    </button>
                    <button onClick={() => updatePremium(c.id, !c.is_premium)} style={{ flex: 1, padding: "7px", borderRadius: 8, background: c.is_premium ? "var(--accent-dim)" : "var(--bg-surface)", border: `1px solid ${c.is_premium ? "var(--accent)" : "var(--border)"}`, color: c.is_premium ? "var(--accent)" : "var(--text-muted)", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
      ...
      {tab === "payments" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            {[
              { label: "Total Revenue", value: `${payments.filter(p => p.status === "completed").reduce((s, p) => s + p.amount, 0).toFixed(0)} EGP`, color: "var(--accent)" },
              { label: "Transactions", value: payments.length, color: "var(--cyan)" },
              { label: "Premium Subs", value: payments.filter(p => p.type === "premium").length, color: "var(--amber)" },
              { label: "Coach Members", value: payments.filter(p => p.type === "coach_membership").length, color: "var(--blue)" },
            ].map(s => (
                  ...
                  <div key={p.id} style={{ padding: "16px 20px", borderBottom: i < arr.length - 1 ? "1px solid var(--border)" : "none", display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
                    <div style={{ flex: 1, minWidth: 200 }}>
                      ...
                      <p style={{ fontSize: 12, color: "var(--text-secondary)", marginTop: 3 }}>
                        {p.type === "premium" ? "User Premium" : "Coach Membership"} · {p.plan} · <strong style={{ color: "var(--accent)" }}>{p.amount?.toFixed(0)} EGP</strong>
                      </p>
                      {p.proof_url && (
          ...
          <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 16, overflow: "hidden" }}>
            <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--border)" }}>
              <p style={{ fontFamily: "'Chakra Petch', sans-serif", fontSize: 14, fontWeight: 700 }}>Payment Transactions</p>
            </div>
            {payments.length === 0 ? (
              <div style={{ padding: 40, textAlign: "center", color: "var(--text-muted)", fontSize: 14 }}>No payments recorded yet.</div>
            ) : (
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 600 }}>
                  <thead>
                    <tr style={{ backgroundColor: "var(--bg-surface)" }}>
                      {["User", "Type", "Method", "Amount", "Status", "Date"].map(h => (
                        <th key={h} style={{ padding: "10px 16px", textAlign: "left", fontSize: 11, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.07em", borderBottom: "1px solid var(--border)" }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {payments.map((p, i) => (
                      <tr key={p.id} style={{ borderBottom: i < payments.length - 1 ? "1px solid var(--border)" : "none" }}>
                        ...
                        <td style={{ padding: "12px 16px", fontSize: 12, color: p.type === "premium" ? "var(--accent)" : "var(--cyan)" }}>{p.type === "premium" ? "User Premium" : "Coach Member"}</td>
                        ...
                        <td style={{ padding: "12px 16px" }}>
                          <span style={{ fontSize: 11, padding: "3px 9px", borderRadius: 20, fontWeight: 600,
                            ...
                            {p.status === "completed" ? "✓ Paid" : p.status === "pending" ? "⏳ Pending" : "✗ Rejected"}
                          </span>
                        </td>
      ...
      {tab === "ads" && (() => {
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {filteredAds.length === 0 ? (
            <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 16, padding: 40, textAlign: "center", color: "var(--text-muted)", fontSize: 14 }}>
              {ads.length === 0 ? "No ads submitted yet." : `No ${currentFilter} ads.`}
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {filteredAds.map(ad => {
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
                          ...
                          <div style={{ padding: "10px 12px", borderRadius: 10, background: "var(--bg-surface)", border: "1px solid var(--border)" }}>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: 3, fontSize: 12 }}>
                              <span>Amount: <strong style={{ color: "var(--amber)" }}>{ad.paid_amount || 0} EGP</strong></span>
                              <span>Duration: <strong style={{ color: "var(--accent)" }}>{ad.paid_minutes || 0} min</strong></span>
                              <span>Status: <strong style={{ color: ad.payment_status === "approved" ? "var(--accent)" : ad.payment_status === "pending" ? "var(--amber)" : "var(--red)" }}>{ad.payment_status || "unpaid"}</strong></span>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {gifts.length === 0 ? (
            <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 16, padding: 40, textAlign: "center", color: "var(--text-muted)" }}>
              ...
              <p style={{ fontFamily: "'Chakra Petch', sans-serif" }}>No gifts sent yet</p>
              <p style={{ fontSize: 13, marginTop: 4 }}>Send points, premium access to athletes</p>
            </div>
          ) : (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 16, padding: "20px 22px" }}>
            ...
            {[
              ...
              { label: "Premium Members", value: users.filter(u => u.is_premium).length },
              { label: "Total Revenue", value: `${payments.filter(p => p.status === "completed").reduce((s, p) => s + p.amount, 0).toFixed(0)} EGP` },
              { label: "Pending Ads", value: ads.filter(a => a.status === "pending").length },
            ].map(s => (
          ...
          <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 16, padding: "20px 22px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
              ...
              <div style={{ padding: "16px", backgroundColor: "rgba(0,220,200,0.06)", border: "1px solid rgba(0,220,200,0.2)", borderRadius: 12 }}>
                <p style={{ fontSize: 13, fontWeight: 700, color: "var(--cyan)", marginBottom: 12 }}>🏋️ Coach Membership — PayPal Credentials</p>
                <p style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 12, lineHeight: 1.5 }}>
          ...
          <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 20, padding: 28, width: "100%", maxWidth: 440 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <div>
                  <label style={{ fontSize: 11, color: "var(--text-muted)", display: "block", marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.07em" }}>Type</label>
