...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    // Block coaches without paid membership (admins exempt)
    const fullUserCheck = await get<any>('SELECT role, membership_paid, coach_membership_active FROM users WHERE id = ?', [user.id]);
    if (fullUserCheck?.role === 'coach' && !fullUserCheck?.membership_paid && !fullUserCheck?.coach_membership_active) {
      return res.status(403).json({ 
        message: 'Coach membership required. Please subscribe to access the coach dashboard.',
        code: 'COACH_MEMBERSHIP_REQUIRED'
      });
    }
