  ...
  try {
    ...
    for (const acc of accounts) {
      ...
      if (!existing) {
        ...
        await run(
          'INSERT INTO users (email, password, name, role, avatar, is_premium, membership_paid, points, steps) VALUES (?, ?, ?, ?, ?, 1, 1, ?, ?)',
          [acc.email, hash, acc.name, acc.role, avatar, acc.points, acc.steps]
        );
