...
router.get('/users', authenticateToken, adminOnly, async (_req: Request, res: Response) => {
  try {
    const users = await query('SELECT id, name, email, role, avatar, is_premium, points, steps, height, weight, gender, membership_paid, coach_membership_active, created_at FROM users ORDER BY created_at DESC');
    res.json({ users });
...
router.post('/gifts', authenticateToken, adminOnly, async (req: any, res: Response) => {
  ...
  if (type === 'premium') await run('UPDATE users SET is_premium = 1 WHERE id = ?', [user_id]);
