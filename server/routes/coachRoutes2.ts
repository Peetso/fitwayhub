...
router.post('/ads', authenticateToken, coachOrAdmin, uploadVideo.fields([{ name: 'image', maxCount: 1 }, { name: 'video', maxCount: 1 }]), async (req: any, res: Response) => {
...
// ── Coach: update ad ─────────────────────────────────────────────────────────
router.put('/ads/:id', authenticateToken, coachOrAdmin, uploadVideo.fields([{ name: 'image', maxCount: 1 }, { name: 'video', maxCount: 1 }]), async (req: any, res: Response) => {
  const { id } = req.params;
  const { title, description, specialty, cta, highlight, paymentMethod, ad_type, media_type, objective, duration_hours, duration_days } = req.body;
  try {
...
router.post('/users/:id/workout-plan', authenticateToken, coachOrAdmin, async (req: any, res: Response) => {
  const { id } = req.params;
...
router.post('/users/:id/nutrition-plan', authenticateToken, coachOrAdmin, async (req: any, res: Response) => {
  const { id } = req.params;
...
router.patch('/users/:id/step-goal', authenticateToken, coachOrAdmin, async (req: any, res: Response) => {
  const { id } = req.params;
...
router.post('/ads/:id/payment', authenticateToken, coachOrAdmin, async (req: any, res: Response) => {
  const { id } = req.params;
